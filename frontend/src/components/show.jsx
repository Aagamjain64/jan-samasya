...
const Show = () => {
  ...
  const handleDelete = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`${BASE_URL}/problem/${problem._id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert("🗑️ Deleted");
      navigate('/');
  ...
  if (!problem) {
    return (
      <div className="show-page container py-4">
        <p>❌ No problem data received.</p>
        <button type="button" className="btn btn-outline-secondary mt-3" onClick={() => navigate(-1)}>
          ← Back
        </button>
      </div>
    );
  ...
  return (
    <div className="show-page container py-4">
      ...
      <div className="card-poster-bar mb-2" style={{ maxWidth: '600px' }}>
        <span className="card-poster-label">Posted by</span>
        <span className="card-poster-name">
          {problem.postedByUsername || problem.PostedBy?.username || 'Unknown user'}
        </span>
      </div>
